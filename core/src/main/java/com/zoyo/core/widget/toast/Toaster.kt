...
object Toaster {
    ...
    private fun build(
        ...
        val currentToast: Toast = Toast(BaseApplication.getContext())

        val contentView = LayoutInflater.from(BaseApplication.getContext()).inflate(R.layout.toast_layout, null)

        val toastText = contentView.findViewById<TextView>(R.id.text)
        toastText.text = text
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            toastText.setTextColor(
                BaseApplication.getContext().resources.getColor(
                    textColor,
                    BaseApplication.getContext().theme
                )
            )
        }
